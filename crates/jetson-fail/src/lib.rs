//! This crate provides a protocol for Jetson's error handling, including a subtrait
...
//!
//! # The `JetsonFail` trait
//!
//! The main interface for Jetson errors is `JetsonFail`, which extends the
...
//!
//! The `JetsonFail::is_user_friendly()` method determines whether an error type is
//! intended for being presented to the end-user. The top-level logic of Jetson uses
//! this to create a single catch-all behavior to present any non-user-friendly errors
//! as an internal error.
//!
//! ## Exit codes
//!
//! The `JetsonFail::exit_code()` method allows each error type to indicate what the
//! process exit code should be if the error is the reason for exiting Jetson.
//!
//! # The `JetsonError` type and `Fallible` functions
//!
//! The main error type provided by this crate is `JetsonError`. This acts more
//! or less as the "root" error type for Jetson; all Jetson error types can be
...
//! `Result<T, JetsonError>` (where `T` is whatever type you want for successful
...
//! # struct Rgb { r: u8, g: u8, b: u8 }
//!
//! // same as: fn parse_component(src: &str, i: usize) -> Result<u8, JetsonError>
...
//!
//! To create an error type in Jetson, add a `#[derive]` attribute to derive the `Fail`
...
//! implementation of `JetsonFail` for the type where `is_user_friendly` returns `true`
...
//! ```
//! # use failure::Fail;
//! # use jetson_fail::{ExitCode, Fallible, JetsonFail};
//! use jetson_fail::throw;
//!
//! # use jetson_fail_derive::*;
//! # #[derive(Debug, Fail, JetsonFail)]
//! # #[fail(display = "unexpected end of string")]
...
//! fn parse_component(src: &str, i: usize) -> Fallible<u8> {
//!     if i + 2 > src.len() {
//!         // UnexpectedEndOfString implements JetsonFail, so it coerces to JetsonError
...
//! need to be converted to Jetson errors. Since third party libraries have not been
//! designed with Jetson's end-user error messages in mind, third-party error types are
//! not automatically converted into Jetson errors.
//!
...
//! method will convert any third-party error to a Jetson error. The resulting Jetson
...
//!     }
//!
//!     // convert the std::num::ParseIntError into a JetsonError
...
//! error message as part of a _cause_ chain, which Jetson's top-level can then use
...
/// Exit codes supported by the JetsonFail trait.
...
/// The failure trait for all Jetson errors.
pub trait JetsonFail: Fail {
...
/// The `JetsonError` type, which can contain any Jetson failure.
#[derive(Debug)]
pub struct JetsonError {
...
impl fmt::Display for JetsonError {
...
impl JetsonError {
    ...
    /// Attempts to downcast this error to a particular `JetsonFail` type by reference.
    ///
    /// If the underlying error is not of type `T`, this will return `None`.
    pub fn downcast_ref<T: JetsonFail>(&self) -> Option<&T> {
        self.error.downcast_ref()
    }

    /// Attempts to downcast this error to a particular `JetsonFail` type by mutable reference.
    ///
    /// If the underlying error is not of type `T`, this will return `None`.
    pub fn downcast_mut<T: JetsonFail>(&mut self) -> Option<&mut T> {
...
impl<T: JetsonFail> From<T> for JetsonError {
...
/// to be converted to a Jetson error. This marks the error as an unknown error, i.e.
/// a non-user-friendly error.
pub trait FailExt {
    fn unknown(self) -> JetsonError;
    fn with_context<F, D>(self, f: F) -> JetsonError
    where
        F: FnOnce(&Self) -> D,
        D: JetsonFail;
}
...
/// or other lower-layer errors into Jetson errors.
pub trait ResultExt<T, E> {
    /// Convert any error-producing result into a `JetsonError`-producing result.
    fn unknown(self) -> Result<T, JetsonError>;
    ...
    fn with_context<F, D>(self, f: F) -> Result<T, JetsonError>
    where
        F: FnOnce(&E) -> D,
        D: JetsonFail;
}
...
#[derive(JetsonFail)]
#[jetson_fail(code = "UnknownError", friendly = "false")]
struct UnknownJetsonError {
    error: failure::Error,
}
...
impl fmt::Debug for UnknownJetsonError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Debug::fmt(self.error.as_fail(), f)
    }
}

impl Display for UnknownJetsonError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "An unknown error has occurred")
    }
}

impl Fail for UnknownJetsonError {
    fn cause(&self) -> Option<&dyn Fail> {
        Some(self.error.as_fail())
    }
...
impl<E: Into<failure::Error>> FailExt for E {
    fn unknown(self) -> JetsonError {
        UnknownJetsonError { error: self.into() }.into()
    }
...
impl<T, E: Into<failure::Error>> ResultExt<T, E> for Result<T, E> {
    fn unknown(self) -> Result<T, JetsonError> {
        self.map_err(|err| UnknownJetsonError { error: err.into() }.into())
    }
...
impl<D: JetsonFail> JetsonFail for failure::Context<D> {
    fn is_user_friendly(&self) -> bool {
...
/// A convenient shorthand for `Result` types that produce `JetsonError`s.
pub type Fallible<T> = Result<T, JetsonError>;
