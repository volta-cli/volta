//! The view layer of Jetson, with utilities for styling command-line output.
use crate::error::ErrorContext;
use archive::Origin;
use console::{style, StyledObject};
use failure::Fail;
use indicatif::{ProgressBar, ProgressStyle};
use jetson_fail::JetsonError;
use term_size;
...
Jetson is still a pre-alpha project, so we expect to run into some bugs,
but we'd love to hear about them so we can fix them!

Please feel free to reach out to us at \x1b[36m\x1b[1m@jetsonjs\x1b[0m on Twitter or file an issue at:

    \x1b[1mhttps://github.com/jetson-cli/jetson/issues\x1b[0m
";

/// Format an error for output in the given context
pub(crate) fn format_error_message(cx: ErrorContext, err: &JetsonError) -> String {
...
fn error_prefix(cx: ErrorContext) -> StyledObject<&'static str> {
    match cx {
        ErrorContext::Jetson => {
            // Since the command here was `jetson`, it would be redundant to say that this was
            // a Jetson error, so we are less explicit in the heading.
            style("error:").red().bold()
        }
        ErrorContext::Shim => {
            // Since a Jetson error is rare case for a shim, it can be surprising to a user.
            // To make it extra clear that this was a failure that happened in Jetson when
            // attempting to delegate to a shim, we are more explicit about the fact that it's
            // a Jetson error.
            style("Jetson error:").red().bold()
        }
    }
}
