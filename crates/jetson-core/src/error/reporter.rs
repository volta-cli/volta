...
const JETSON_DEV: &'static str = "JETSON_DEV";

/// Represents the context from which an error is being reported.
pub enum ErrorContext {
    /// An error reported from the `jetson` executable.
    Jetson,
...
pub struct ErrorReporter {
    /// Jetson version to display in error logs
    version: String,
...
impl ErrorReporter {
    ...
    pub fn from_flag(jetson_version: &str, verbose: bool) -> Self {
        if verbose {
            ErrorReporter {
                version: jetson_version.to_string(),
                verbose,
            }
        } else {
            ErrorReporter::from_env(jetson_version)
        }
    }

    /// Create a new ErrorReporter from the environment variables
    pub fn from_env(jetson_version: &str) -> Self {
        ErrorReporter {
            version: jetson_version.to_string(),
            verbose: env::var(JETSON_DEV).is_ok(),
        }
    }

    /// Report an error, both to the terminal and the error log
    pub fn report(&self, cx: ErrorContext, err: &JetsonError) {
    ...
    fn write_error_log(&self, message: String, details: String) -> Result<String, Error> {
        let file_name = Local::now()
            .format("jetson-error-%Y-%m-%d_%H_%M_%S%.3f.log")
            .to_string();
        ...
        writeln!(log_file, "Jetson v{}", self.version)?;
        writeln!(log_file)?;
...
fn compose_error_details(err: &JetsonError) -> Option<String> {
