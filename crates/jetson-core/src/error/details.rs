...
variables `JETSON_DEV` set to `1` and `RUST_BACKTRACE` set to `full`, and open
an issue at https://github.com/jetson-cli/jetson/issues with the details!";

const PERMISSIONS_CTA: &'static str =
    "Please ensure you have correct permissions to the Jetson directory.";
...
pub enum ErrorDetails {
    ...
    /// Thrown when a user tries to `jetson pin` something other than node/yarn/npm.
    CannotPinPackage {
        package: String,
    },
    ...
    /// Thrown when Jetson is unable to create a shim
    ShimCreateError {
        name: String,
    },
    ...
    /// Thrown when Jetson is unable to remove a shim
    ShimRemoveError {
        name: String,
    },
    ...
    /// Thrown when the shell name specified in the Jetson environment is not supported.
    UnrecognizedShell {
        name: String,
    },

    /// Thrown when the postscript file was not specified in the Jetson environment.
    UnspecifiedPostscript,

    /// Thrown when the shell name was not specified in the Jetson environment.
    UnspecifiedShell,
...
See `jetson help install` and `jetson help pin` for info about making tools available."),
            ErrorDetails::BinaryNotFound { name } => write!(f, r#"Could not find executable "{}"

Use `jetson install` to add a package to your toolchain (see `jetson help install` for more info)."#, name),
...
Use `jetson install` to add a package to your toolchain (see `jetson help install` for more info)."
            ),
...
Please ensure the environment variable 'HOME' is set.")
            ...
            ErrorDetails::NoInstallDir => write!(f, "Could not determine Jetson install directory.

Please ensure Jetson was installed correctly"),
...
Use `jetson pin node` to pin Node first, then pin a Yarn version."
            ),
            ErrorDetails::NoPlatform => write!(
                f,
                "Could not determine Node version.

Use `jetson pin node` to select a version for a project.
Use `jetson install node` to select a default version."
            ),
            ErrorDetails::NoProjectYarn => write!(
                f,
                "No Yarn version found in this project.

Use `jetson pin yarn` to select a version (see `jetson help pin` for more info)."
            ),
            ErrorDetails::NotInPackage => write!(f, "Not in a node package.

Use `jetson install` to select a default version of a tool."),
            ErrorDetails::NoUserYarn => write!(
                f,
                "Could not determine Yarn version.

Use `jetson install yarn` to select a default version (see `jetson help install for more info)."
            ),
...
Please ensure you are using a supported shell.", name),
            ErrorDetails::UnspecifiedPostscript => {
                write!(f, "Could not determine Jetson postscript file.

Please ensure Jetson was installed correctly.")
            }
            ErrorDetails::UnspecifiedShell => write!(f, "Jetson shell not specified"),
...
impl JetsonFail for ErrorDetails {
