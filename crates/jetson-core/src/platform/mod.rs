...
impl Image {
    ...
    /// for the given versions instead of in the Jetson shim directory.
...
impl System {
    /// Produces a modified version of the current `PATH` environment variable that
    /// removes the Jetson shims and binaries, to use for running system node and
    /// executables.
    ...
    /// Reproduces the Jetson-enabled `PATH` environment variable for situations where
    /// Jetson has been deactivated
...
mod test {
    ...
    use crate::path::{jetson_home, shim_dir};
    use semver::Version;
    use std;
    use std::path::PathBuf;
