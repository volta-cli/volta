...
//! in a standard Jetson layout in Unix-based operating systems.
...
use super::{node_archive_root_dir_name, node_image_dir, jetson_home, shim_dir};
...
pub fn jetson_file() -> Fallible<PathBuf> {
    Ok(jetson_home()?.join("jetson"))
}

pub fn shim_executable() -> Fallible<PathBuf> {
    Ok(jetson_home()?.join("shim"))
}
