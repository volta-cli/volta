...
//! in a standard Jetson layout in Windows operating systems.
...
// wix/main.wxs -
const JETSON_REGISTRY_PATH: &'static str = r#"Software\The Jetson Maintainers\Jetson"#;

// This Key needs to exactly match the Name from the above element in the Windows Installer
const JETSON_INSTALL_DIR: &'static str = "InstallDir";
...
//         jetson.exe                                      jetson_file
//         shim.exe                                        shim_executable
//         hooks.toml                                      user_hooks_file
//
// C:\Program Files\
//     Jetson\                                             (Path stored in Windows Registry by installer)
//         bin\
//             jetson.exe                                  jetson_file
...
pub fn default_jetson_home() -> Fallible<PathBuf> {
...
cfg_if::cfg_if! {
    ...
    if #[cfg(test)] {
        ...
        fn install_dir() -> Fallible<PathBuf> {
            ...
            let jetson_key = hklm.open_subkey(JETSON_REGISTRY_PATH).with_context(install_dir_error)?;
            let install_path: String = jetson_key.get_value(JETSON_INSTALL_DIR).with_context(install_dir_error)?;
...
pub fn jetson_file() -> Fallible<PathBuf> {
    Ok(install_bin_dir()?.join("jetson.exe"))
}
