...
fn fetch_node() {
    ...
    assert_that!(p.jetson("fetch node 10.4.1"), execs().with_status(0));
...
fn fetch_yarn() {
    ...
    assert_that!(p.jetson("fetch yarn 1.10.1"), execs().with_status(0));
...
fn fetch_npm() {
    ...
    assert_that!(p.jetson("fetch npm 6.7.0"), execs().with_status(0));
...
fn fetch_package() {
    ...
    assert_that!(p.jetson("fetch cowsay 1.4.0"), execs().with_status(0));
